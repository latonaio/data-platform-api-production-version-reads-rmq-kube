...
import (
	dpfm_api_caller "data-platform-api-production-version-reads-rmq-kube/DPFM_API_Caller"
	...
	"data-platform-api-production-version-reads-rmq-kube/config"
	"encoding/json"
	"fmt"
	"time"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	res, errs := caller.AsyncProductionVersionReads(accepter, &input, &output, l)
	if len(errs) != 0 {
